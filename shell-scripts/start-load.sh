...
do
  ...
  curl -i http://$(svcIp.kubectl)/api/v1/claims/
  # sleep 1

  ## Add a claim for institutional provider 1
  echo -e "*****Inserting claim [$counter] for institutional provider 1"
  clmid=`curl -X POST -H "Content-Type: application/json" -k -d @./claim01.json http://$(svcIp.kubectl)/api/v1/claims/ | jq-linux64 '.claimItemId'`
  # Get & update the claim
  echo -e "*****Retrieving and updating the claim for institutional provider 1"
  record=`curl -k http://$(svcIp.kubectl)/api/v1/claims/$clmid | sed -e "s/1234.50/4321.50/g"`
  echo
  ...
  curl -X PUT -H "Content-Type: application/json" -k --data "$record" http://$(svcIp.kubectl)/api/v1/claims/$clmid
  echo
  echo -e "*****Retrieving the updated institutional claim"
  # Get the claim
  curl -k https://$(svcIp.kubectl)/api/v1/claims/$clmid
  ...
  clmid=`curl -X POST -H "Content-Type: application/json" -k -d @./claim02.json https://$(svcIp.kubectl)/api/v1/claims/ | jq-linux64 '.claimItemId'`
  # Get & update the claim
